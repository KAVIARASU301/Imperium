...
Standalone dialog showing Price (top) + CVD (bottom) with:
  ...
  • 1D / 2D toggle                     (inside Setup panel)
  • Timeframe selector: 1m | 3m | 5m | 15m | 30m
  • Chart type toggle: line ↔ candlestick (inside Setup panel)
  • EMA 10 / 21 toggles                (inside Setup panel)
  • VWAP toggle                        (inside Setup panel)
  • CVD Rebased toggle                 (inside Setup panel)
  ...
  • 2D mode: fixed X range anchored at mid-session, grows rightward
"""
...
from PySide6.QtCore import Qt, QThread, QTimer, QRectF, QPointF, QPoint, QEvent
...
def _hsep():
    """Thin horizontal rule for the setup panel."""
    line = QFrame()
    line.setFrameShape(QFrame.HLine)
    line.setFixedHeight(1)
    line.setStyleSheet("background: #1E2D40; border: none;")
    return line

def _section_label(text):
    lbl = QLabel(text)
    lbl.setStyleSheet(
        "color:#3A4A60;font-size:9px;font-weight:700;letter-spacing:1px;"
...
class _OHLCItem(pg.GraphicsObject):
    def __init__(self, bull_color: str, bear_color: str, alpha: int = 225):
        ...
        bull_fill = QColor(bull_qc); bull_fill.setAlpha(alpha)
        bear_fill = QColor(bear_qc); bear_fill.setAlpha(alpha)
...
# ---------------------------------------------------------------------------
class _SetupPanel(QWidget):
    """Compact floating panel for indicators & chart settings."""
    ...
    def __init__(self, parent: QWidget):
        ...
        # ── Chart type ──────────────────────────────────────────────────────
        root.addWidget(_section_label("CHART TYPE"))
        row_ct = QHBoxLayout(); row_ct.setSpacing(4)
        self.btn_line    = _btn("Line",    fg=_C["text_1"], mw=52, checkable=True,
                                chk_bg="#2A3B5C", chk_fg="#FFFFFF")
        self.btn_candles = _btn("Candles", fg=_C["text_1"], mw=60, checkable=True,
                                chk_bg="#26A69A", chk_fg="#000")
        row_ct.addWidget(self.btn_line)
        row_ct.addWidget(self.btn_candles)
        row_ct.addStretch()
        root.addLayout(row_ct)

        root.addWidget(_hsep())

        # ── Mode ────────────────────────────────────────────────────────────
        root.addWidget(_section_label("MODE"))
        row_mode = QHBoxLayout(); row_mode.setSpacing(4)
        self.btn_1d = _btn("1D", fg=_C["text_1"], mw=34, checkable=True,
                           chk_bg="#26A69A", chk_fg="#000")
        self.btn_2d = _btn("2D", fg=_C["text_1"], mw=34, checkable=True,
                           chk_bg="#26A69A", chk_fg="#000")
        row_mode.addWidget(self.btn_1d)
        row_mode.addWidget(self.btn_2d)
        row_mode.addStretch()
        root.addLayout(row_mode)

        row_cvd = QHBoxLayout(); row_cvd.setSpacing(4)
        self.btn_cvd_rebase = _btn("↺ Rebased CVD", fg=_C["text_2"], mw=110,
                                   checkable=True, chk_bg="#2A3B5C", chk_fg="#FFFFFF")
        row_cvd.addWidget(self.btn_cvd_rebase)
        row_cvd.addStretch()
        root.addLayout(row_cvd)

        root.addWidget(_hsep())

        # ── Indicators ──────────────────────────────────────────────────────
        root.addWidget(_section_label("INDICATORS"))
        row_ema = QHBoxLayout(); row_ema.setSpacing(4)
        lbl_ema = QLabel("EMA")
        lbl_ema.setStyleSheet(
            "color:#6A7A90;font-size:10px;font-weight:700;background:transparent;")
        row_ema.addWidget(lbl_ema)
        ...
        for cb in (self.cb_ema10, self.cb_ema21, self.cb_ema51):
            row_ema.addWidget(cb)
        row_ema.addStretch()
        root.addLayout(row_ema)

        row_vwap = QHBoxLayout(); row_vwap.setSpacing(4)
        row_vwap.addSpacing(32)
        self.cb_vwap = _ema_cb("VWAP", _C["vwap"])
        row_vwap.addWidget(self.cb_vwap)
        row_vwap.addStretch()
        root.addLayout(row_vwap)
...
class PriceCVDChartDialog(QDialog):
    ...
    def __init__(self, kite, instrument_token: int, symbol: str, cvd_engine=None, parent=None):
        ...
        self._selected_tf     = 3             # default: 3m
        self._use_ohlc        = True          # default: candles
    ...
    def _setup_ui(self):
        ...
        self._build_setup_panel()   # overlay widget, initially hidden
        self._build_charts(root)
        # Apply defaults after all widgets exist
        self._init_defaults()
    ...
    def _build_toolbar(self, root):
        ...
        self.lbl_dates.setMinimumWidth(260)
        ...
        self.cb_tf.setStyleSheet("""
            QComboBox {
                background: #151D2B; color: #8A99B3;
                border: 1px solid #1E2D40; border-radius: 4px;
        ...
        row.addWidget(self.cb_tf)

        self.btn_setup = _btn("⚙  Setup", fg=_C["text_2"], mw=70, checkable=True,
                              chk_bg="#1C2638", chk_fg=_C["text_1"])
        self.btn_setup.setToolTip("Open indicator & chart settings")
        self.btn_setup.clicked.connect(self._toggle_setup_panel)
    ...
    def _build_setup_panel(self):
        """Create the floating setup panel (child overlay, initially hidden)."""
        self._setup_panel = _SetupPanel(self)
        self._setup_panel.hide()

        # Wire up controls — bind references for the rest of the dialog
        p = self._setup_panel
        self.btn_1d         = p.btn_1d
        self.btn_2d         = p.btn_2d
        self.btn_chart_style = p.btn_candles   # kept for _set_chart_style compat
        self.btn_cvd_rebase  = p.btn_cvd_rebase
        self.cb_ema10        = p.cb_ema10
        self.cb_ema21        = p.cb_ema21
        self.cb_ema51        = p.cb_ema51
        self.cb_vwap         = p.cb_vwap

        p.btn_1d.clicked.connect(lambda: self._set_day_mode(False))
        p.btn_2d.clicked.connect(lambda: self._set_day_mode(True))
        p.btn_line.clicked.connect(lambda: self._on_chart_style_toggled(False))
        p.btn_candles.clicked.connect(lambda: self._on_chart_style_toggled(True))
        p.btn_cvd_rebase.toggled.connect(self._on_cvd_rebase_toggled)
        for cb in (p.cb_ema10, p.cb_ema21, p.cb_ema51, p.cb_vwap):
            cb.toggled.connect(self._on_indicator_toggled)

    def _init_defaults(self):
        """Set all initial states (called after all widgets are created)."""
        # TF: 3m
        for i in range(self.cb_tf.count()):
            if self.cb_tf.itemData(i) == self._selected_tf:
                self.cb_tf.blockSignals(True)
                self.cb_tf.setCurrentIndex(i)
                self.cb_tf.blockSignals(False)
                break

        # 2D mode, candles, CVD rebased — order matters
        self._set_day_mode(True, reload=False)
        self._set_chart_style(True)            # candles
        self._setup_panel.btn_cvd_rebase.setChecked(True)

    def _toggle_setup_panel(self, checked: bool):
        if not checked:
            self._setup_panel.hide()
            return
        # Position below the setup button, right-aligned
        btn_br   = self.btn_setup.mapTo(self, QPoint(self.btn_setup.width(), self.btn_setup.height()))
        px = btn_br.x() - self._setup_panel.width()
        py = btn_br.y() + 4
        px = max(4, min(px, self.width() - self._setup_panel.width() - 4))
        py = max(0, min(py, self.height() - self._setup_panel.height() - 4))
        self._setup_panel.move(px, py)
        self._setup_panel.show()
        self._setup_panel.raise_()
    ...
    def _build_charts(self, root):
        ...
        self.price_plot.showGrid(x=True, y=True, alpha=0.07)
        ...
        py_right.setWidth(68)
        py_right.setTextPen(pg.mkPen(_C["price"]))
        py_right.setPen(pg.mkPen(_C["border"]))
        py_right.enableAutoSIPrefix(False)

        self._price_prev       = pg.PlotCurveItem(pen=pg.mkPen(_C["price_prev"], width=1.5, style=Qt.DashLine))
        ...
        self._price_level_badge.setStyleSheet(
            ...
            "font-size:11px;font-weight:700;padding:1px 4px;border-radius:2px;")
        ...
        self.cvd_plot.showGrid(x=True, y=True, alpha=0.07)
        ...
        self._cvd_prev       = pg.PlotCurveItem(pen=pg.mkPen(_C["cvd_prev"], width=1.5, style=Qt.DashLine))
        ...
        # Hide all overlays — they're shown on first data render
        for curve in (self._price_ema10, self._cvd_ema10,
                      self._price_ema21, self._cvd_ema21,
                      self._price_ema51, self._cvd_ema51,
                      self._price_vwap):
            curve.hide()
    ...
    def _set_chart_style(self, use_ohlc: bool):
        ...
        with suppress(Exception):
            p.btn_line.blockSignals(True)
            p.btn_candles.blockSignals(True)
            p.btn_line.setChecked(not use_ohlc)
            p.btn_candles.setChecked(use_ohlc)
            p.btn_line.blockSignals(False)
            p.btn_candles.blockSignals(False)

    def _on_chart_style_toggled(self, use_ohlc: bool):
        self._chart_style_overridden = True
        self._set_chart_style(use_ohlc)
        if self._all_timestamps:
            self._render_from_cache()
    ...
    def _render_from_cache(self):
        ...
        if not self._two_day:
            ...
            # Fixed 1D X range: 9:15 → 15:30, candles grow left-to-right
            ...
            # ── Fixed 2D X range: anchored at 0, chart mid = session break  ─
            # Morning: right half empty.  Grows rightward as session progresses.
            bars_per_session = max(1, MINUTES_PER_SESSION // tf)
            total_slots      = bars_per_session * 2
            self.cvd_plot.setXRange(0, total_slots - 1 + offset, padding=0.02)
    ...
    def _position_badge(self, plot, axis, value, badge):
        """
        Accurately map a data-space Y value → screen pixel → badge position.
        Uses viewportTransform (handles all pyqtgraph scaling/pan internally)
        then accounts for viewport offset within the outer PlotWidget.
        """
        ...
        # View-space → scene → viewport pixel via transform
        scene_pt   = vb.mapViewToScene(QPointF(0, clamped))
        vp_transform = plot.viewportTransform()
        vp_pt      = vp_transform.map(scene_pt)

        # Viewport is a child widget of PlotWidget; add its offset
        local_y = vp_pt.y() + plot.viewport().pos().y()

        axis_width = max(0, int(axis.width()))
        x = max(0, plot.width() - axis_width)

        y = int(local_y - badge.height() / 2)
